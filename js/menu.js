...
function renderCategories(categories) {
    ...
    Object.keys(categories).forEach((categoryName, index) => {
        ...
        categoryElement.innerHTML = `
            <img width="200px" src="img/pasta.svg" alt="Breakfast" class="menu-category-img">
            <div class="menu-category-info">
                <h3 class="menu-category-title">${categoryName}</h3>
                <p class="text-gray">${categories[categoryName].length}</p>
            </div>
        `
    ...
    items.forEach((item) => {
        ...
        menuItemElement.innerHTML = `
            <div class="menu-item-header">
                <img width="200px" src="${item.image}" alt="${item.name}" class="menu-item-img">
                <div class="menu-item-title">
                    <h3>${item.name}</h3>
                    <p class="text-gray menu-item-desc">Lorem ipsum dolor sit amet.</p>
                </div>
            </div>
...
function addToCart(selectedFood) {
    ...
    if(exisitingItemIndex !== -1) {
        cart[exisitingItemIndex].quantity += 1;
    } else {
        cart.push({
...
function renderInvoice() {
    ...
    cart.forEach((item, index) => {
        ...
        invoiceItemElement.innerHTML = `
            <img width="200px" src="${item.image}" alt="${item.name}" class="invoice-item-img">
